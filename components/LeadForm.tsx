...
export function LeadForm({
  ...
  const [shakeField, setShakeField] = useState<keyof LeadFormFormState | null>(null);
  const stepAnnouncerRef = useRef<HTMLDivElement | null>(null);
  const fieldRefs = useRef<Record<keyof LeadFormFormState, HTMLElement | null>>({
    fullName: null,
    email: null,
    country: null,
    phone: null,
  ...
  const COPY = useMemo(() => {
        ...
        steps: [
          {
            ...
            fields: [
              { id: "fullName", label: "الاسم و اللقب", required: true },
              { id: "email", label: "البريد الإلكتروني", type: "email", required: true },
              { id: "country", label: "البلاد الي تعيش فيها", required: true },
              { id: "phone", label: "رقم الهاتف (واتساب)", type: "tel" },
          ...
          },
          {
            ...
            fields: [
              { id: "bestContactTime", label: "أحسن الأوقات إلي ننجمو نكلموك", required: true },
              { id: "psychologistBefore", label: "مشيت لطبيب نفساني قبل؟ إن كان نعم: التشخيص", required: true },
              { id: "medicationNow", label: "تاخذ في دواء؟", required: true },
            ],
          },
          {
            id: "motivation",
            ...
            fields: [
              { id: "whyCoaching", label: "علاش إخترت تعمل كونتشينڨ؟", type: "textarea", required: true },
              { id: "followingDuration", label: "قداش عندك إتبع في البرهوم", required: true },
              {
                id: "maritalStatus",
              ...
              { id: "occupation", label: "المهنة", required: true },
              { id: "passphrase", label: "كلمة السر", type: "textarea" },
            ],
          },
        ] as StepConfig[],
        next: "التالي",
    ...
    return {
      ...
      privacy: "Your information is confidential and shared only with Barhoum’s office.",
      steps: [
        {
          id: "basic",
          title: "Basic info",
        ...
        },
        {
          ...
          fields: [
            { id: "bestContactTime", label: "Best time to contact", required: true },
            {
              id: "psychologistBefore",
              label: "Seen a psychologist before? If yes, diagnosis",
              required: true,
            },
            { id: "medicationNow", label: "Are you taking medication now?", required: true },
          ],
        },
        {
          id: "motivation",
          ...
          fields: [
            { id: "whyCoaching", label: "Why did you choose coaching?", type: "textarea", required: true },
            { id: "followingDuration", label: "How long following Barhoum?", required: true },
            {
              id: "maritalStatus",
            ...
            { id: "occupation", label: "Occupation", required: true },
            { id: "passphrase", label: "Passphrase", type: "textarea" },
          ],
        },
      ] as StepConfig[],
      next: "Next",
  ...
  const logValidationBlock = (stepIndex: number, fieldId: keyof LeadFormFormState) => {
    console.warn(`[LeadForm] Step ${stepIndex + 1} blocked by field "${fieldId}"`);
  };
  ...
  const handleSubmit = (eventRef: FormEvent<HTMLFormElement>) => {
    ...
    if (!validation.isValid) {
      validation.invalidFieldIds.forEach((fieldId) => logValidationBlock(step, fieldId));
  ...
  const stepIsValid = useMemo(() => {
    ...
    return stepConfig.fields.every((field) => !validateField(field, values[field.id]));
  }, [step, steps, validateField, values]);

  const progressLine = locale === "ar"
  ...
  const handleFieldChange = (fieldId: keyof LeadFormFormState, value: string) => {
    ...
    return (
      ...
      >
        ...
        <header className="space-y-2 text-text">
          <h3 className="text-[clamp(1.9rem,2.6vw,2.2rem)] font-heading font-semibold">{COPY.thankYouTitle}</h3>
          <p className="text-base text-subtle/90">{COPY.thankYouBody}</p>
        </header>
        <div className="rounded-[18px] border border-border/40 bg-background/30 px-5 py-4 text-start text-sm text-subtle">
          <p className="font-medium text-text/90">{COPY.summaryTitle}</p>
  ...
  return (
    <form
      className="relative mx-auto flex w-full max-w-[700px] flex-col gap-8 rounded-[24px] border border-border/35 bg-surface px-6 py-8 shadow-[0_26px_48px_-26px_rgba(15,23,42,0.45)]"
    ...
    >
      ...
      <input type="hidden" name="selectedPackage" value={hiddenValues.pack} />
      <div ref={stepAnnouncerRef} aria-live="polite" className="sr-only" />
      ...
      {showInternalHeader ? (
        <header className="space-y-4 text-text">
          ...
          <p id={helperDescriptionId} className="text-base leading-relaxed text-subtle/80">
            {currentStep.helper}
          </p>
        </header>
      ) : (
        <div className="space-y-3 text-text">
          ...
          <p id={helperDescriptionId} className="text-sm leading-relaxed text-subtle/80">
            {currentStep.helper}
          </p>
        </div>
      )}

      <fieldset className="grid gap-6" aria-describedby={helperDescriptionId}>
          ...
          if (field.options) {
            return (
              <div key={field.id} className="grid gap-2">
                <label htmlFor={field.id} className="text-[0.92rem] font-semibold text-subtle/90">
                  {field.label}
                  {field.required ? " *" : ""}
                </label>
                <select
                  {...commonProps}
                  ref={(node) => {
                    fieldRefs.current[field.id] = node ?? null;
                  }}
                  className={clsx(
                    "w-full rounded-[16px] border border-border/55 bg-white px-4 py-3.5 text-base text-text transition-all duration-200 ease-out focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30",
                    error && "border-primary/60",
                    shakeField === field.id && "animate-[leadform-shake_220ms_ease]"
                  )}
                ...
                {error && (
                  <span role="alert" className="text-xs text-primary/80">
                    {error}
                  </span>
                )}
              </div>
            ...
            return (
              <div key={field.id} className="grid gap-2">
                ...
                <textarea
                  ...
                  ref={(node) => {
                    fieldRefs.current[field.id] = (node as HTMLElement | null) ?? null;
                  }}
                  className={clsx(
                    "min-h-[140px] w-full rounded-[16px] border border-border/55 bg-white px-4 py-3.5 text-base text-text transition-all duration-200 ease-out focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30",
                    error && "border-primary/60",
                    shakeField === field.id && "animate-[leadform-shake_220ms_ease]"
                  )}
                  aria-invalid={Boolean(error)}
          ...
          return (
            <div key={field.id} className="grid gap-2">
              ...
              <input
                ...
                dir={field.id === "email" || field.id === "phone" ? "ltr" : commonProps.dir}
                onBlur={() => handleFieldBlur(field.id)}
              />
              {error && (
                <span role="alert" className="text-xs text-primary/80">
      ...
      <div
        className={clsx(
          "sticky bottom-0 mt-4 flex flex-wrap items-center gap-3 self-stretch border-t border-border/30 bg-surface px-1 pt-4",
      ...
      >
        {step > 0 && (
          <Button
            type="button"
            variant="ghost"
            className="basis-full sm:basis-auto sm:min-w-[140px]"
        ...
        {step < totalSteps - 1 && (
          <Button
            type="button"
            className="basis-full sm:basis-auto sm:min-w-[160px]"
        ...
        {step === totalSteps - 1 && (
          <Button
            type="submit"
            className="basis-full sm:basis-auto sm:min-w-[180px]"
      ...
      <p className="text-start text-xs text-subtle/70">{COPY.privacy}</p>
      <style jsx>{`
        @keyframes leadform-shake {
          0%, 100% { transform: translateX(0); }
          25% { transform: translateX(${isRtl ? "-4px" : "4px"}); }
          75% { transform: translateX(${isRtl ? "4px" : "-4px"}); }
        }
      `}</style>
    </form>
  );
