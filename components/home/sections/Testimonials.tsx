...
export const HomeTestimonials: HomeThemeDefinition["Testimonials"] = ({ 
  ...
  }, [isDragging, handlePrevious, handleNext, isRTL]);
  ...
  const renderTestimonialCard = (testimonial: any, index: number, position: 'center' | 'peek-left' | 'peek-right' = 'center') => {
    ...
    return (
      <article
        ...
        className={`
          relative bg-white rounded-xl transition-all duration-300 ease-out flex-shrink-0
          ${isCenter ? 'scale-100 opacity-100 z-10' : ''}
          ${isPeek ? 'scale-95 opacity-60 z-0' : ''}
          ${isCenter ? 'shadow-[0_8px_32px_rgba(0,0,0,0.08),0_2px_12px_rgba(0,0,0,0.04)]' : 'shadow-[0_4px_16px_rgba(0,0,0,0.04)]'}
        `}
        style={{
          minHeight: '420px',
      ...
      >
        ...
        <div className="flex flex-col items-center text-center h-full justify-between p-8 pt-12">
          ...
          <div className="flex-shrink-0 mb-6">
            <div className="w-16 h-16 rounded-full bg-[#2AD6CA]/8 border-2 border-[#2AD6CA]/20 flex items-center justify-center">
          ...
          <div className="flex-1 flex items-center justify-center px-4">
            <blockquote 
              className={`
                text-2xl md:text-3xl leading-[1.5] text-gray-900 font-medium max-w-2xl
                ${isRTL ? 'text-right font-base' : 'text-center'}
              `}
              dir={isRTL ? 'rtl' : 'ltr'}
            >
              <span className="text-gray-400/60 text-xl mr-1">"</span>
              {testimonial.quote}
              <span className="text-gray-400/60 text-xl ml-1">"</span>
            </blockquote>
          </div>

          {/* Attribution */}
          <footer className="flex-shrink-0 space-y-1 mt-6">
            <cite className="not-italic font-semibold text-gray-900 text-xl block">
              {testimonial.name}
            </cite>
            {testimonial.role && (
      ...
      >
        <Container>
          <div className="text-center space-y-12">
            ...
            {/* Single testimonial */}
            <div className="flex justify-center">
              {renderTestimonialCard(testimonials[0], 0, 'center')}
            </div>
          </div>
        </Container>
      </Section>
  ...
  // Multiple testimonials - carousel with peeks
  return (
    <Section 
      id="testimonials" 
      className="relative bg-gradient-to-b from-white via-gray-50/30 to-white"
      role="region"
      aria-label="Customer testimonials"
    >
      <Container>
          ...
          <div className="relative max-w-[1200px] mx-auto overflow-hidden">
            ...
            >
              <div className="flex items-center justify-center gap-6">
                ...
                <div className="hidden lg:block flex-shrink-0" style={{ width: '10%', overflow: 'hidden' }}>
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm border border-gray-200/60 hover:border-[#2AD6CA] hover:bg-white hover:scale-105 transition-all duration-200 flex items-center justify-center text-gray-500 hover:text-[#2AD6CA] focus:outline-none focus:ring-2 focus:ring-[#2AD6CA]/30 z-20 shadow-sm"
            ...
            >
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`
                    w-2.5 h-2.5 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#2AD6CA]/50
                    ${index === currentIndex
                      ? 'bg-[#2AD6CA] scale-125'
