...
const pageMetadata: Record<PageKey, Metadata> = {
  ...
  cv: {
    title: "Résumé",
    description: "Explore Ibrahim ben Abdallah's professional journey, projects, and collaborative impact.",
    openGraph: {
      url: `${siteUrl}/cv`,
    },
  },
  links: {
    title: "Curated Links",
    description: "Follow the articles, talks, and spaces where Ibrahim ben Abdallah is present.",
    openGraph: {
      url: `${siteUrl}/links`,
    },
  },
  pay: {
    ...
    description: "Complete your private coaching or workshop payment with Ibrahim ben Abdallah.",
    openGraph: {
      url: `${siteUrl}/pay`,
    },
  },
...
export function getPageMetadata(key: Exclude<PageKey, "pay">, overrides?: Metadata): Metadata {
...
export function getPayMetadata(payload?: PaySeoInput, overrides?: Metadata): Metadata {
  const paymentMeta: Metadata | undefined = payload
    ? {
        title: `${payload.packageTitle} Payment`,
        description: `Complete payment for ${payload.clientName} · ${payload.amountLabel}. Secure checkout with Ibrahim ben Abdallah.`
          .trim(),
        openGraph: {
          title: `${payload.packageTitle} Payment`,
  ...
  return mergeMetadata(baseMetadata, pageMetadata.pay, paymentMeta, overrides);
}
